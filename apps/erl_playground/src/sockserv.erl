...
handle_info({tcp, _Port, {test_query, automatron_pid}}, State = {ok, #state{socket = Socket, transport = Transport, automatron_pid = AutomatronPid}}) ->
    send_test_pid(AutomatronPid, Transport, Socket),
    {noreply, State};
...
process_packet(undefined, State, _Now) ->
    ...
    State;
process_packet(#req{ type = Type }, State = {ok, #state{socket = Socket, transport = Transport, automatron_pid = Pid}}, _Now)
    when Type =:= test_pid -> 
    send_test_pid(binary:list_to_bin(erlang:pid_to_list(Pid)), Transport, Socket),
    State;
...
send_test_pid(Pid, Transport, Socket) when is_binary(Pid) ->
    Req = #req{
        type = test_pid,
    ...
    Data = utils:add_envelope(Req),
    Transport:send(Socket,Data);
send_test_pid(Msg, _Transport, _Socket) ->
    lager:info("[WARN] received ~p in send_server_message", [Msg]).
