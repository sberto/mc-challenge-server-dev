...
%% enumerated types
-type 'req.type_enum'() :: create_session | server_message | user_request.
...
-export_type(['create_session'/0, 'server_message'/0, 'user_request'/0, 'req'/0, 'envelope'/0]).

-spec encode_msg(#create_session{} | #server_message{} | #user_request{} | #req{} | #envelope{}) -> binary().
encode_msg(Msg) when tuple_size(Msg) >= 1 ->
    encode_msg(Msg, element(1, Msg), []).

-spec encode_msg(#create_session{} | #server_message{} | #user_request{} | #req{} | #envelope{}, atom() | list()) -> binary().
...
encode_msg_user_request(Msg, TrUserData) ->
    encode_msg_user_request(Msg, <<>>, TrUserData).


encode_msg_user_request(#user_request{message = F1},
                        Bin, TrUserData) ->
    begin
        TrF1 = id(F1, TrUserData),
        e_type_string(TrF1, <<Bin/binary, 10>>, TrUserData)
    ...
    if F4 == undefined -> B3;
       true ->
           begin
               ...
               e_mfield_req_user_request_data(TrF4,
                                              <<B3/binary, 34>>,
                                              TrUserData)
           end
    end.
...
e_mfield_req_user_request_data(Msg, Bin, TrUserData) ->
    SubBin = encode_msg_user_request(Msg, <<>>, TrUserData),
    Bin2 = e_varint(byte_size(SubBin), Bin),
    <<Bin2/binary, SubBin/binary>>.
...
decode_msg_create_session(Bin, TrUserData) ->
    dfp_read_field_def_create_session(Bin,
                                      0,
                                      0,
...
dfp_read_field_def_create_session(<<10, Rest/binary>>,
    ...
    d_field_create_session_username(Rest,
                                    Z1,
                                    Z2,
                                    F@_1,
                                    TrUserData);
...
dfp_read_field_def_create_session(Other, Z1, Z2, F@_1,
                                  TrUserData) ->
    dg_read_field_def_create_session(Other,
                                     Z1,
                                     Z2,
                                     F@_1,
                                     TrUserData).
...
dg_read_field_def_create_session(<<1:1, X:7,
    ...
    when N < 32 - 7 ->
    dg_read_field_def_create_session(Rest,
                                     N + 7,
                                     X bsl N + Acc,
                                     F@_1,
                                     TrUserData);
dg_read_field_def_create_session(<<0:1, X:7,
    ...
    case Key of
        ...
        _ ->
            case Key band 7 of
                0 ->
                    skip_varint_create_session(Rest,
                                               0,
                                               0,
                                               F@_1,
                                               TrUserData);
                ...
                2 ->
                    skip_length_delimited_create_session(Rest,
                                                         0,
                                                         0,
                                                         F@_1,
                                                         TrUserData);
                3 ->
                    skip_group_create_session(Rest,
                                              Key bsr 3,
                                              0,
                                              F@_1,
                                              TrUserData);
...
d_field_create_session_username(<<0:1, X:7,
    ...
    dfp_read_field_def_create_session(RestF,
                                      0,
                                      0,
                                      NewFValue,
                                      TrUserData).
...
skip_varint_create_session(<<0:1, _:7, Rest/binary>>,
                           Z1, Z2, F@_1, TrUserData) ->
    dfp_read_field_def_create_session(Rest,
                                      Z1,
                                      Z2,
                                      F@_1,
                                      TrUserData).
...
skip_length_delimited_create_session(<<0:1, X:7,
    ...
    dfp_read_field_def_create_session(Rest2,
                                      0,
                                      0,
                                      F@_1,
                                      TrUserData).
...
decode_msg_server_message(Bin, TrUserData) ->
    dfp_read_field_def_server_message(Bin,
                                      0,
                                      0,
                                      id(undefined, TrUserData),
                                      TrUserData).
...
dfp_read_field_def_server_message(<<10, Rest/binary>>,
                                  Z1, Z2, F@_1, TrUserData) ->
    d_field_server_message_message(Rest,
                                   Z1,
                                   Z2,
                                   F@_1,
                                   TrUserData);
...
dfp_read_field_def_server_message(Other, Z1, Z2, F@_1,
                                  TrUserData) ->
    dg_read_field_def_server_message(Other,
                                     Z1,
                                     Z2,
                                     F@_1,
                                     TrUserData).
...
dg_read_field_def_server_message(<<1:1, X:7,
                                 ...
                                 N, Acc, F@_1, TrUserData)
    when N < 32 - 7 ->
    dg_read_field_def_server_message(Rest,
                                     N + 7,
                                     X bsl N + Acc,
                                     F@_1,
                                     TrUserData);
dg_read_field_def_server_message(<<0:1, X:7,
    ...
    case Key of
        ...
        _ ->
            case Key band 7 of
                0 ->
                    skip_varint_server_message(Rest,
                                               0,
                                               0,
                                               F@_1,
                                               TrUserData);
                ...
                2 ->
                    skip_length_delimited_server_message(Rest,
                                                         0,
                                                         0,
                                                         F@_1,
                                                         TrUserData);
                3 ->
                    skip_group_server_message(Rest,
                                              Key bsr 3,
                                              0,
                                              F@_1,
                                              TrUserData);
...
d_field_server_message_message(<<0:1, X:7,
    ...
    dfp_read_field_def_server_message(RestF,
                                      0,
                                      0,
                                      NewFValue,
                                      TrUserData).
...
skip_varint_server_message(<<0:1, _:7, Rest/binary>>,
                           Z1, Z2, F@_1, TrUserData) ->
    dfp_read_field_def_server_message(Rest,
                                      Z1,
                                      Z2,
                                      F@_1,
                                      TrUserData).
...
skip_length_delimited_server_message(<<0:1, X:7,
    ...
    Length = X bsl N + Acc,
    <<_:Length/binary, Rest2/binary>> = Rest,
    dfp_read_field_def_server_message(Rest2,
                                      0,
                                      0,
                                      F@_1,
                                      TrUserData).
...
decode_msg_user_request(Bin, TrUserData) ->
    dfp_read_field_def_user_request(Bin,
                                    0,
                                    0,
                                    id(undefined, TrUserData),
                                    TrUserData).

dfp_read_field_def_user_request(<<10, Rest/binary>>, Z1,
                                Z2, F@_1, TrUserData) ->
    d_field_user_request_message(Rest,
                                 Z1,
                                 Z2,
                                 F@_1,
                                 TrUserData);
dfp_read_field_def_user_request(<<>>, 0, 0, F@_1, _) ->
    #user_request{message = F@_1};
dfp_read_field_def_user_request(Other, Z1, Z2, F@_1,
                                TrUserData) ->
    dg_read_field_def_user_request(Other,
                                   Z1,
                                   Z2,
                                   F@_1,
                                   TrUserData).

dg_read_field_def_user_request(<<1:1, X:7,
                                 Rest/binary>>,
                               N, Acc, F@_1, TrUserData)
    when N < 32 - 7 ->
    dg_read_field_def_user_request(Rest,
                                   N + 7,
                                   X bsl N + Acc,
                                   F@_1,
                                   TrUserData);
dg_read_field_def_user_request(<<0:1, X:7,
                                 Rest/binary>>,
                               N, Acc, F@_1, TrUserData) ->
    Key = X bsl N + Acc,
    case Key of
        ...
        _ ->
            case Key band 7 of
                0 ->
                    skip_varint_user_request(Rest, 0, 0, F@_1, TrUserData);
                1 -> skip_64_user_request(Rest, 0, 0, F@_1, TrUserData);
                2 ->
                    skip_length_delimited_user_request(Rest,
                                                       0,
                                                       0,
                                                       F@_1,
                                                       TrUserData);
                3 ->
                    skip_group_user_request(Rest,
                                            Key bsr 3,
                                            0,
                                            F@_1,
                                            TrUserData);
                5 -> skip_32_user_request(Rest, 0, 0, F@_1, TrUserData)
            end
    end;
dg_read_field_def_user_request(<<>>, 0, 0, F@_1, _) ->
    #user_request{message = F@_1}.

d_field_user_request_message(<<1:1, X:7, Rest/binary>>,
                             N, Acc, F@_1, TrUserData)
    when N < 57 ->
    d_field_user_request_message(Rest,
                                 N + 7,
                                 ...
                                 F@_1,
                                 TrUserData);
d_field_user_request_message(<<0:1, X:7, Rest/binary>>,
                             N, Acc, _, TrUserData) ->
    {NewFValue, RestF} = begin
                             Len = X bsl N + Acc,
                             <<Bytes:Len/binary, Rest2/binary>> = Rest,
    ...
    dfp_read_field_def_user_request(RestF,
                                    0,
                                    0,
                                    NewFValue,
                                    TrUserData).

skip_varint_user_request(<<1:1, _:7, Rest/binary>>, Z1,
                         Z2, F@_1, TrUserData) ->
    skip_varint_user_request(Rest,
                             Z1,
                             Z2,
                             F@_1,
                             TrUserData);
skip_varint_user_request(<<0:1, _:7, Rest/binary>>, Z1,
                         Z2, F@_1, TrUserData) ->
    dfp_read_field_def_user_request(Rest,
                                    Z1,
                                    Z2,
                                    F@_1,
                                    TrUserData).

skip_length_delimited_user_request(<<1:1, X:7,
                                     Rest/binary>>,
                                   N, Acc, F@_1, TrUserData)
    when N < 57 ->
    skip_length_delimited_user_request(Rest,
                                       ...
                                       F@_1,
                                       TrUserData);
skip_length_delimited_user_request(<<0:1, X:7,
                                     Rest/binary>>,
                                   N, Acc, F@_1, TrUserData) ->
    Length = X bsl N + Acc,
    <<_:Length/binary, Rest2/binary>> = Rest,
    dfp_read_field_def_user_request(Rest2,
                                    0,
                                    0,
                                    F@_1,
                                    TrUserData).

skip_group_user_request(Bin, FNum, Z2, F@_1,
                        TrUserData) ->
    {_, Rest} = read_group(Bin, FNum),
    dfp_read_field_def_user_request(Rest,
                                    0,
...
skip_32_user_request(<<_:32, Rest/binary>>, Z1, Z2,
                     F@_1, TrUserData) ->
    dfp_read_field_def_user_request(Rest,
                                    Z1,
                                    Z2,
...
skip_64_user_request(<<_:64, Rest/binary>>, Z1, Z2,
                     F@_1, TrUserData) ->
    dfp_read_field_def_user_request(Rest,
                                    Z1,
                                    Z2,
...
d_field_req_user_request_data(<<0:1, X:7, Rest/binary>>,
    ...
    {NewFValue, RestF} = begin
                             Len = X bsl N + Acc,
                             <<Bs:Len/binary, Rest2/binary>> = Rest,
                             {id(decode_msg_user_request(Bs, TrUserData),
                                 TrUserData),
                              Rest2}
                         end,
    dfp_read_field_def_req(RestF,
...
decode_msg_envelope(Bin, TrUserData) ->
    dfp_read_field_def_envelope(Bin,
                                0,
                                0,
                                id(undefined, TrUserData),
                                TrUserData).
...
dfp_read_field_def_envelope(<<18, Rest/binary>>, Z1, Z2,
                            F@_1, TrUserData) ->
    d_field_envelope_uncompressed_data(Rest,
                                       Z1,
                                       Z2,
                                       F@_1,
                                       TrUserData);
...
dfp_read_field_def_envelope(Other, Z1, Z2, F@_1,
                            TrUserData) ->
    dg_read_field_def_envelope(Other,
                               Z1,
                               Z2,
                               F@_1,
                               TrUserData).
...
dg_read_field_def_envelope(<<1:1, X:7, Rest/binary>>, N,
                           Acc, F@_1, TrUserData)
    when N < 32 - 7 ->
    dg_read_field_def_envelope(Rest,
                               N + 7,
                               X bsl N + Acc,
                               F@_1,
                               TrUserData);
    ...
    case Key of
        ...
        _ ->
            case Key band 7 of
                ...
                2 ->
                    skip_length_delimited_envelope(Rest,
                                                   0,
                                                   0,
                                                   F@_1,
                                                   TrUserData);
                3 ->
                    skip_group_envelope(Rest,
                                        Key bsr 3,
                                        0,
                                        F@_1,
                                        TrUserData);
...
d_field_envelope_uncompressed_data(<<0:1, X:7,
    ...
    dfp_read_field_def_envelope(RestF,
                                0,
                                0,
                                if Prev == undefined -> NewFValue;
                                   true ->
...
skip_varint_envelope(<<0:1, _:7, Rest/binary>>, Z1, Z2,
                     F@_1, TrUserData) ->
    dfp_read_field_def_envelope(Rest,
                                Z1,
                                Z2,
                                F@_1,
                                TrUserData).
...
skip_length_delimited_envelope(<<0:1, X:7,
    ...
    Length = X bsl N + Acc,
    <<_:Length/binary, Rest2/binary>> = Rest,
    dfp_read_field_def_envelope(Rest2,
                                0,
                                0,
                                F@_1,
                                TrUserData).
...
-compile({nowarn_unused_function,merge_msg_user_request/3}).
merge_msg_user_request(#user_request{},
...
merge_msg_req(#req{create_session_data =
    ...
    #req{type = NFtype,
         ...
         user_request_data =
             if PFuser_request_data /= undefined,
                NFuser_request_data /= undefined ->
                    merge_msg_user_request(PFuser_request_data,
                                           NFuser_request_data,
                                           TrUserData);
...
merge_msg_envelope(#envelope{uncompressed_data =
                  ...
                  merge_msg_req(PFuncompressed_data,
                                NFuncompressed_data,
                                TrUserData)}.
...
-compile({nowarn_unused_function,v_msg_user_request/3}).
-dialyzer({nowarn_function,v_msg_user_request/3}).
v_msg_user_request(#user_request{message = F1}, Path,
                   TrUserData) ->
    v_type_string(F1, [message | Path], TrUserData),
    ok;
...
v_msg_req(#req{type = F1, create_session_data = F2,
    ...
    if F2 == undefined -> ok;
       true ->
           v_msg_create_session(F2,
                                [create_session_data | Path],
                                TrUserData)
    end,
    if F3 == undefined -> ok;
       true ->
           v_msg_server_message(F3,
                                [server_message_data | Path],
                                TrUserData)
    end,
    if F4 == undefined -> ok;
       true ->
           v_msg_user_request(F4,
                              [user_request_data | Path],
                              TrUserData)
    end,
    ok;
